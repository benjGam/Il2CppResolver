...
internal sealed class Il2CppClassLayoutDetector
{
    ...
    public Il2CppClassLayoutDetector(Il2CppTarget target, IReadOnlyList<Il2CppClassLayout> candidates, int minimumValidatedClassCount = 3)
    {
        ...
        HashSet<(int StaticFieldsPointerOffset, int StaticFieldsSizeOffset)> structures = new();
        ...
        foreach (Il2CppClassLayout candidate in candidates)
        {
            ...
            if (!structures.Add((candidate.StaticFieldsPointerOffset, candidate.StaticFieldsSizeOffset)))
                throw new ArgumentException($"Class-layout compatibility profile '{candidate.Name}' duplicates an existing structural identity.", nameof(candidates));
        }
